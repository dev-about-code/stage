   ...
   public JsonMessage deserialize(String message) throws IOException {
      ...
      if (messageClass == null) {
         throw new IOException(
             String.format("Could not find a matching object type for message: %s", message));
      } else {
         try {
            return gson.fromJson(parseResult.two(), messageClass);
         } catch (Exception e) {
            throw new IOException(String.format("Could not deserialize type '%s' from message: %s",
