...
public class ApplicationContainer {
   ...
   /**
    * Runs the specified application with the specified arguments as a deamon. I.e. after the method
    * {@link ComponentContainer#start()} has been called, the application will wait for an explicit
    * call to {@link ApplicationContainer#shutdown(String)}.
    *
    * @param application               The application to run
    * @param arguments                 The arguments to configure the application with
    * @param applicationStatusListener The listener that should be notified for application events
   ...
   private void init(Application application) {
      ...
      configurationParameters.add(ConfigurationParameter.File("configuration-file",
             ...
             for (Map.Entry<String, List<String>> entry : configurationProperties.entrySet()) {
   ...
   private void printUsage() {
      ...
      for (ConfigurationParameter configurationParameter : parameters.stream()
         ...
         usageInformation.append(String
                                     .format(
                                         "  --%-" + maxNameLength + "s [%-1s] %-" + (maxTypeLength
                                                                                     + 2)
                                         + "s -> %s\n",
                                         configurationParameter.getName(),
   ...
   private int doRun(Application application) {
      ...
      try {
         ...
         logger.error(
             String.format("Could not parse application arguments because: %s", e.getMessage()), e);
         return EXIT_ERROR;
      }
   ...
   private class DefaultApplicationConfigurationContext implements ApplicationConfigurationContext {
      ...
      public void addConfigurationParameter(
         ...
         notifyStartupEvent("Startup", 10, processedActions, String
             .format("Adding configuration parameter '%s'", configurationParameter.getName()));
         configurationParameters.add(configurationParameter);
      }
