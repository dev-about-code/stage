...
public class ComponentContainer {
        ...
        synchronized (monitor) {
            if (!stateMachine.isInInitialState()) {
                throw new IllegalStateException(String.format(
                        "Trying to add component '%s' to container after initialization",
                        identifier));
            }
    ...
    private class ComponentController implements ComponentContext {
        ...
        void destroy() {
            try {
                transitState(LifeCycleStatus.Destroying, LifeCycleStatus.Destroyed,
                             component::destroy);
            } catch (LifeCycleException e) {
                // never thrown
            }
        ...
        public void requestTermination(String reason, Exception cause) {
            logger.warn(String
                                .format("Component requests termination '%s' because: %s",
                                        getIdentifier(),
                                        reason), cause);
