   ...
   static Neo4JDatabaseConfiguration bolt(String serverName, int port, String username,
      ...
      try {
         ...
         throw new IllegalArgumentException(
             String.format("Could not create connection url because: %s", e.getMessage()), e);
      }
      return () -> GraphDatabase.driver(uri, AuthTokens.basic(username, password), config);
   }

   /**
    * Creates the database {@link Driver} with the corresponding parameters for the underlying Neo4J
    * database.
    *
